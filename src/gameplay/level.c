...
void draw_level_tiles(const Level* level) {
    ...
    // retângulo de origem no atlas para blocos sólidos
    const Rectangle platform_src =
        (assets) ? assets->rect_platform : (Rectangle){0,0,ts,ts};

    for (int y = 0; y < level->height; y++) {
        for (int x = 0; x < level->width; x++) {
            if (level->tiles[y][x] == TILE_SOLID) {
                Vector2 pos = (Vector2){ x * ts, y * ts };

                if (assets) {
                    DrawSprite(platform_src, pos);
                } else {
                    // Fallback simples
                    DrawRectangle((int)pos.x, (int)pos.y, (int)ts, (int)ts, GRAY);
                    DrawRectangleLines((int)pos.x, (int)pos.y, (int)ts, (int)ts, BLACK);
                }
            }
        }
    }
